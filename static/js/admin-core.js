...
let D={students:[],classes:[],registrations:[],payments:[],gallery:[],testimonials:[],contacts:[],events:[],users:[],announcements:[],banners:[]};
...
{id:'gallery',icon:'bi-camera-fill',label:'Galeri'},
{id:'testimonials',icon:'bi-chat-heart-fill',label:'Testimoni'},
{id:'locations',icon:'bi-geo-alt-fill',label:'Lokasi Sanggar'},
{id:'contacts',icon:'bi-envelope-fill',label:'Kontak Masuk'},
{t:'Lainnya'},
{id:'_website',icon:'bi-globe2',label:'Lihat Website',href:'../index.html'},
...
function checkAuth(){if(sessionStorage.getItem('admin_logged_in')!=='true'){fetch(API+'/check_auth.php',{credentials:'include'}).then(r=>r.json()).then(d=>{if(!d.logged_in)location.href='./admin-login.html'}).catch(()=>location.href='./admin-login.html')}document.getElementById('sidebarUsername').textContent=sessionStorage.getItem('admin_username')||'Admin'}
